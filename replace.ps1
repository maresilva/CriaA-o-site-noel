...
$content = $content -replace '(?s)<title>.*?</title>', '<title>CriaAção Entretenimento | Soluções Natalinas Exclusivas</title>'
...
$content = $content -replace '(?s)<!-- MAIN CONTENT: QUEM SOMOS -->\s*<main id="quem-somos-main">.*?</main>', $newMain

[System.IO.File]::WriteAllText("solucoes.html", $content, [System.Text.Encoding]::UTF8)
